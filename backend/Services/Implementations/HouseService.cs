...
namespace backend.Services.Implementations
{
    public class HouseService : IHouseService
    {
        ...
        public async Task<IReadOnlyCollection<HouseAggregator>> GetAllHouseSensors(int page, int pageSize, int limit_sensors, int offset_sensors)
        {
            if(_authenticatedUserService.UserId == null)
                throw new UnauthorizedAccessException("You are not authorized to get all houses");
            ...
            {
                var houseAggregator = new HouseAggregator(house, await _sensorRepository.GetTemperatureByHouseIdAsync(house.Id, offset_sensors, limit_sensors), await _sensorRepository.GetHumidityByHouseIdAsync(house.Id, offset_sensors, limit_sensors),await _sensorRepository.GetSunExposureByHouseIdAsync(house.Id, offset_sensors, limit_sensors));
        ...
        public async Task<House> UpdateHouse(Guid id, HouseAggregator house)
        {
            ...
            House houseToUpdate  = await _houseRepository.GetByIdAsync(id);
            ...
            var houseUpdate = new House() {
                Id = id,
                Name = house.House.Name != houseToUpdate.Name ? house.House.Name : houseToUpdate.Name,
                Address = house.House.Address != houseToUpdate.Address ? house.House.Address : houseToUpdate.Address,
                City = house.House.City != houseToUpdate.City ? house.House.City : houseToUpdate.City,
                CreatedAt = houseToUpdate.CreatedAt,
                State = house.House.State != houseToUpdate.State ? house.House.State : houseToUpdate.State,
            };
            ...
            if(house.Temperatures.Count() > 0) {
                foreach (var temperature in house.Temperatures)
                {
                    ...
                    var sensorUpdate = new Sensor() {
                        ...
                        Value = temperature.Value != sensorToUpdate.Value ? temperature.Value : sensorToUpdate.Value,
                    };
            ...
            if(house.Humidities.Count() > 0) {
                foreach (var humidity in house.Humidities)
                {
                    ...
                    var sensorUpdate = new Sensor() {
                        ...
                        HouseId = sensorToUpdate.HouseId,
                        Type = sensorToUpdate.Type,
                        Value = humidity.Value != sensorToUpdate.Value ? humidity.Value : sensorToUpdate.Value,
                    };
                    sensorsToUpdate.Add(sensorUpdate);
                }
            }
            if(house.SunExposures.Count() > 0) {
                foreach (var sunExposure in house.SunExposures)
                {
                    ...
                    var sensorUpdate = new Sensor() {
                        ...
                        HouseId = sensorToUpdate.HouseId,
                        Type = sensorToUpdate.Type,
                        Value = sunExposure.Value != sensorToUpdate.Value ? sunExposure.Value : sensorToUpdate.Value,
                    };
                    sensorsToUpdate.Add(sensorUpdate);
                }
            }
