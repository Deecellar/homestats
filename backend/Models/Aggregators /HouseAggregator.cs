...
{
    public HouseAggregator(House house, IEnumerable<Temperature> temperatures, IEnumerable<Humidity> humidities,
        IEnumerable<SunExposure> sunExposures) : this(house.Id, house.CreatedAt, house.Name, house.Address, house.City,
        house.State, temperatures, humidities, sunExposures)
    {
    }

    public HouseAggregator(Guid id, DateTime createdAt, string name, string address, string city, string state,
        IEnumerable<Temperature> temperatures, IEnumerable<Humidity> humidities, IEnumerable<SunExposure> sunExposures)
    {
        ...
        if (temperatures != null)
        {
            ...
            if (temperatures.Any(t =>
                    t.Value < -100 ||
                    t.Value > 60)) // Austrailia proved 50 is not the max and also california it seems. Antarctica is -89.2 C soo...
        ...
        {
            if (temperatures.Any(t => t.HouseId != id) || humidities.Any(h => h.HouseId != id) ||
                sunExposures.Any(s => s.HouseId != id))
                exceptions.Add(new DomainException("House id must be the same for all devices"));
            if (temperatures.Any(t => t.RecordedAt < createdAt) || humidities.Any(h => h.RecordedAt < createdAt) ||
                sunExposures.Any(s => s.RecordedAt < createdAt))
        ...
        House = new House(name, address, city, state)
        {
            CreatedAt = createdAt
        };
...
public class HouseResponse
{
    ...
    public HouseAggregator ToHouseAggregator()
    {
        return new HouseAggregator(Id, CreatedAt, Name, Address, City, State, Temperatures, Humidities, SunExposures);
    }
}